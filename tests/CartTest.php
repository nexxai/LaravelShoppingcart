...
class CartTest extends TestCase
{
    ...
    public function it_can_search_the_cart_for_a_specific_item()
    {
        ...
        $cartItem = $cart->search(function ($cartItem, $rowId) {
            return 'Some item' == $cartItem->name;
        });
    ...
    public function it_can_search_the_cart_for_a_specific_item_with_options()
    {
        ...
        $cartItem = $cart->search(function ($cartItem, $rowId) {
            return 'red' == $cartItem->options->color;
        });
     ...
     * @test
     */
    public function it_will_not_throw_an_exception_when_a_cart_was_already_stored_using_the_specified_identifier()
    {
        $this->artisan('migrate', [
            '--database' => 'testing',
        ]);
    ...
    public function it_can_restore_a_cart_from_the_database()
    {
        ...
        $restored = $cart->restore($identifier);

        $this->assertItemsInCart(1, $cart);

        $this->assertDatabaseHas('shoppingcart', ['identifier' => $identifier, 'instance' => 'default']);
        $this->assertInstanceOf(\Gloudemans\Shoppingcart\Cart::class, $restored);
     ...
     * @test
     */
    public function it_will_throw_an_exception_if_a_cart_id_was_not_found()
    {
        $this->expectException(\Gloudemans\Shoppingcart\Exceptions\InvalidCartIdentifierException::class);
        $this->expectExceptionMessage('There is no known cart with ID: abcxyz');

        $this->artisan('migrate', [
            '--database' => 'testing',
        ]);

        \Gloudemans\Shoppingcart\Facades\Cart::restore('abcxyz');
    }

    /**
     * @test
     */
    public function it_can_be_erased()
    {
        $this->expectException(\Gloudemans\Shoppingcart\Exceptions\InvalidCartIdentifierException::class);
